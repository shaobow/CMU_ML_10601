...
def errorTrain(trainDataArray,maxDepth):
    trainedDT=trainDecisionTree(trainDataArray,maxDepth)
    trainDataPrediction=predictLabel(trainedDT,trainDataArray)    
    error=errorRate(trainDataArray,trainDataPrediction)
    return error

def errorTest(trainDataArray,testDataArray,maxDepth):
    trainedDT=trainDecisionTree(trainDataArray,maxDepth)
    testDataPrediction=predictLabel(trainedDT,testDataArray)
    error=errorRate(testDataArray,testDataPrediction)
    return error
...
x=np.arange(attributeNum)
y_train=[errorTrain(trainDataArray,x_i) for x_i in x]
y_test=[errorTest(trainDataArray,testDataArray,x_i) for x_i in x]

plt.figure(1,dpi=300)
plt.plot(x,y_train,label='Train_Error')
plt.plot(x,y_test,label='Test_Error')
plt.legend()
plt.xlabel("Depth of the tree(k)")
plt.ylabel("Error rate")
plt.title("Error rate against depth plot for politicians datas")
...
# python decisionTree.py politicians_train.tsv politicians_test.tsv 4 pol_4_train.labels pol_4_test.labels pol_4_metrics.txt
...
# python decisionTree.py mushroom_train.tsv mushroom_test.tsv 7 mushroom_7_train.labels mushroom_7_test.labels mushroom_7_metrics.txt